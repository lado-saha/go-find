...
import { StolenItem } from '@/app/lib/models';
import { fetchFile } from '@/app/lib/data';

export default async function StolenItemEditForm({
  stolenItem,
  photoFiles,
}: {
  ...
  const [tags, setTags] = useState<string[]>(
    stolenItem.tags.map((tag) => {
      return tag.name;
    }),
  );
  const [inputValue, setInputValue] = useState<string>('');
  const inputRef = useRef<HTMLInputElement>(null);
  const [photosFiles, setPhotosFiles] = useState<File[]>(photoFiles);
